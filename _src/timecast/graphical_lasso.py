...
def init_precision(emp_cov, mode="empirical"):
    ...
    if mode == "empirical":
        ...
        for i, (c, e) in enumerate(zip(covariance_, emp_cov)):
            c.flat[:: n_features + 1] = e.flat[:: n_features + 1]  # 対角成分を代入
            # import scipy
            # K[i] = scipy.linalg.pinvh(c) #empirical covarianceの対角成分を除いた成分を*0.95した行列の擬似逆行列を初期値として使用する
...
convergence = namedtuple_with_defaults(
    "convergence", "obj obj_simple rnorm snorm e_pri e_dual precision"
)
...
# def logl_refined(emp_cov, precision):
#     """Gaussian log-likelihood without constant term."""
#     logl = fast_logdet(precision) - np.sum(emp_cov * precision)
#     if  is None:
...
def objective_simple(n_samples, S, K, alpha):
    """Objective function for time-varying graphical lasso."""
    obj = loss(S, K, n_samples=n_samples)

    if isinstance(alpha, np.ndarray):
        obj += sum(l1_od_norm(a * z) for a, z in zip(alpha, K))
    else:
...
def _graphical_lasso(
    ...
    checks = [
        convergence(
            ...
            obj_simple=objective_simple(n_samples, emp_cov, Z_0, alpha),
        )
    ]
    for iteration_ in range(max_iter):
        ...
        A += emp_cov  
        # TODO:-A+Sにしてる、A-Sでは？そのせいかなprox_logdetで、dのところが、-dになってる
        ...
        obj_simple = (
            objective_simple(n_samples, emp_cov, Z_0, alpha)
            if compute_objective
            else np.nan
        )
        ...
        if verbose:
            print(
                "obj: %.4f, obj_simple: %.4f, rnorm: %.4f, snorm: %.4f,"
                "eps_pri: %.4f, eps_dual: %.4f" % check[:6]
            )
...
class graphical_lasso(object):
    ...
    def _fit(self, emp_cov, n_samples=None, init_inv_cov=None):
    ...
    def fit(self, X):
        ...
        Parameters
        ----------
        X : ndarray, shape = (n_samples * duration, n_dimensions)
            Data matrix.
        """
        ...
        emp_cov = np.array([empirical_covariance(X, assume_centered=self.assume_centered)])
        self.emp_cov = emp_cov[0]
        gl_model = self._fit(emp_cov,n_samples)
        
        return gl_model


    def fit_additional_data(self, X, ns):
        """Fit the TimeGraphicalLasso model to X.
        ...
        n_dimensions = X.shape[1]
        current_n_samples = X.shape[0]
        
        # n_samples = None
        # self.classes_, n_samples = np.unique(y, return_counts=True)
        mean_add = np.mean(X,axis=0)
        emp_cov_add = empirical_covariance(X, assume_centered=self.assume_centered)
        merged_emp_cov = self.merge_covariances(ns, self.mean_, self.emp_cov, current_n_samples, mean_add, emp_cov_add)
        n_samples = current_n_samples+ ns
        gl_model = self._fit(np.array([merged_emp_cov]),np.array([n_samples]))
        gl_model.emp_cov = copy.deepcopy(merged_emp_cov)
        return gl_model

    # coveriance type
    def merge_covariances(self, n1, mean1, cov1, n2, mean2, cov2):
        total_n = n1 + n2
        mean = (n1 * mean1 + n2 * mean2) / total_n
        
        mean_diff = (mean1 - mean2).reshape(-1, 1)
        mean_correction = (n1 * n2) / total_n * (mean_diff @ mean_diff.T)
        
        den_bias=0
        merged_cov = ((n1 - den_bias) * cov1 + (n2 - den_bias) * cov2 + mean_correction) / (total_n - den_bias)
